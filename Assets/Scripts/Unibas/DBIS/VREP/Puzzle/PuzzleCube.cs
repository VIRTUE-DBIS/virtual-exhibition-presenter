...
namespace Unibas.DBIS.VREP.Puzzle
{
    public class PuzzleCube : MonoBehaviour
    {
        ...
        {
            int[] ids = new int[neighborsToCheck()];
            int i = 0;
            if (doTestTop)
            {
                ids[i++] = PuzzleManager.GetInstance().GetIdForPos(expectedTop);
            }

            if (doTestRight)
            {
                ids[i++] = PuzzleManager.GetInstance().GetIdForPos(expectedRight);
            }

            if (doTestBottom)
            {
                ids[i++] = PuzzleManager.GetInstance().GetIdForPos(expectedBottom);
            }

            if (doTestLeft)
            {
                ids[i++] = PuzzleManager.GetInstance().GetIdForPos(expectedLeft);
            }
        ...
        {
            int i = 0;
            foreach (int id in GetNeighborIds())
            {
                var other = PuzzleManager.GetInstance().GetForId(id);
                // Basically very arbitrary if the distance is less than size and a little bit (a fouth of size), consider it okay.
                if (Vector3.Distance(transform.position, other.transform.position) - (Size + Size / 4f) < 0.01f)
                {
                    i++;
                }
            }

            return i >= neighborsToCheck();
        }

        public void RestoreInitialNeighborChecks()
        {
            Debug.Log("PuzzleCube " + Id + " .. restoring neighbors b,l,t,r " + (PuzzlePos.y == 0) + "," +
                      (PuzzlePos.x == 0) + "," + (PuzzlePos.y == NbCubes.y - 1) + "," +
                      (+PuzzlePos.x == NbCubes.x - 1));
        ...
        public void OnSidedTriggerEnter(PuzzleSide side, PuzzleCubeSide collidedWith)
        {
                ...
                case PuzzleSide.LEFT:
                    if (doTestLeft)
                    {
                        leftNeighbor = side == PuzzleSide.LEFT && collidedWith.Side == PuzzleSide.RIGHT &&
                                       expectedLeft == collidedWith.PuzzleCube.PuzzlePos;
                ...
                case PuzzleSide.RIGHT:
                    if (doTestRight)
                    {
                        rightNeighbor = side == PuzzleSide.RIGHT && collidedWith.Side == PuzzleSide.LEFT &&
                                        expectedRight == collidedWith.PuzzleCube.PuzzlePos;
                ...
                case PuzzleSide.TOP:
                    if (doTestTop)
                    {
                        topNeighbor = side == PuzzleSide.TOP && collidedWith.Side == PuzzleSide.BOTTOM &&
                                      expectedTop == collidedWith.PuzzleCube.PuzzlePos;
                ...
                case PuzzleSide.BOTTOM:
                    if (doTestBottom)
                    {
                        bottomNeighbor = side == PuzzleSide.BOTTOM && collidedWith.Side == PuzzleSide.TOP &&
                                         expectedBottom == collidedWith.PuzzleCube.PuzzlePos;
            ...
            Debug.Log("My nb correct neighbors: " + Id + "(" + PuzzlePos.x + "," + PuzzlePos.y + ") : " +
                      correctNeighbors() + " of " + neighborsToCheck());
            PuzzleManager.GetInstance()
                .SetPositionCheck(PuzzlePos.x, PuzzlePos.y, correctNeighbors() >= neighborsToCheck());
        }
        ...
        public void OnSidedTriggerExit(PuzzleSide side, PuzzleCubeSide collidedWith)
        {
            ...
            PuzzleManager.GetInstance()
                .SetPositionCheck(PuzzlePos.x, PuzzlePos.y,
                    correctNeighbors() >= neighborsToCheck()); // TODO more sophistcated?
        }
